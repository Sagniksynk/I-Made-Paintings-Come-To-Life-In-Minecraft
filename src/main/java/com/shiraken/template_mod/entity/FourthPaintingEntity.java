...
public class FourthPaintingEntity extends PathfinderMob implements GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final EntityDataAccessor<Boolean> IS_ACTIVATED = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> ACTIVATION_TICK = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.INT);
    
    // Add SynchedEntityData for the target coordinates so server dictates where the entity goes
    private static final EntityDataAccessor<Float> TARGET_X = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> TARGET_Y = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> TARGET_Z = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);

    // Save initial spawn position to reset properly
    private static final EntityDataAccessor<Float> SPAWN_X = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> SPAWN_Y = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> SPAWN_Z = SynchedEntityData.defineId(FourthPaintingEntity.class, EntityDataSerializers.FLOAT);

    private static final int ANIM_TICKS = 90;
    ...
    public FourthPaintingEntity(EntityType<? extends PathfinderMob> type, Level level) {
        super(type, level);
    }
    ...
    public void triggerReset() {
        if (!this.level().isClientSide) {
            ...
            // Reset frame
            java.util.List<FourthPhotoEntity> frames = this.level().getEntitiesOfClass(FourthPhotoEntity.class, this.getBoundingBox().inflate(32.0D));
    ...
    public void setActivated(boolean activated) {
        ...
        if (activated) {
            ...
            if (!this.level().isClientSide) {
                ...
                java.util.List<FourthPhotoEntity> frames = this.level().getEntitiesOfClass(FourthPhotoEntity.class, this.getBoundingBox().inflate(32.0D));
                FourthPhotoEntity closestFrame = null;
                ...
                if (closestFrame != null) {
                    ...
                    // Direction vector points outwards. Target should perfectly match the Blockbench Z translation (-34.76206 / 16.0 = 2.1726 blocks).
                    this.entityData.set(TARGET_X, (float)(closestFrame.getX() + dir.getStepX() * 2.1726D));
                    this.entityData.set(TARGET_Y, (float)this.getY());
                    this.entityData.set(TARGET_Z, (float)(closestFrame.getZ() + dir.getStepZ() * 2.1726D));
                } else {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new WaterAvoidingRandomStrollGoal(this, 1.0D) {
            ...
            public boolean canUse() {
                return FourthPaintingEntity.this.isActivated() && !FourthPaintingEntity.this.isLocked && super.canUse();
            }
        });
    ...
    public void tick() {
        ...
        if (this.isActivated()) {
            ...
            if (ticksSinceActivation < TOTAL_LOCKED_TICKS) {
                ...
                // We just sit inside the painting until the landing animation finishes.
                super.tick();
    ...
    private PlayState predicate(AnimationState<FourthPaintingEntity> event) {
        if (!this.isActivated()) {
        ...
        if (ticksSinceActivation < 90) {
            event.getController().setAnimationSpeed(1.0D); // Set speed to 1x to match exact 4 seconds
    ...
    class FollowPlayerGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            if (!FourthPaintingEntity.this.isActivated()) return false;
            if (FourthPaintingEntity.this.isLocked) return false;

            this.targetPlayer = FourthPaintingEntity.this.level().getNearestPlayer(FourthPaintingEntity.this, 64.0D);
            return this.targetPlayer != null && this.targetPlayer.isAlive() && !this.targetPlayer.isSpectator();
        }

        @Override
        public boolean canContinueToUse() {
            if (!FourthPaintingEntity.this.isActivated() || FourthPaintingEntity.this.isLocked) return false;
            return this.targetPlayer != null && this.targetPlayer.isAlive() && !this.targetPlayer.isSpectator();
        }
        ...
        public void tick() {
            if (this.targetPlayer != null) {
                FourthPaintingEntity.this.getLookControl().setLookAt(this.targetPlayer, 30.0F, 30.0F);

                if (--this.delayCounter <= 0) {
                    this.delayCounter = 10;
                    FourthPaintingEntity.this.getNavigation().moveTo(this.targetPlayer, 0.8D);
                }
            }
        }
    }
