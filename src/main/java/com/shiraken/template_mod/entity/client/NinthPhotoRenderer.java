...
import com.shiraken.template_mod.entity.NinthPhotoEntity;
...
public class NinthPhotoRenderer extends EntityRenderer<NinthPhotoEntity> {
    public static int SWAP_TIME_IN_TICKS = 2; // 0.1 seconds into break animation (~0.08s = 1.6 ticks)

    private static final ResourceLocation MATCH_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/ninth_match.png");
    private static final ResourceLocation MATCH_AFTER_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/ninth_match_after.png");

    public NinthPhotoRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(NinthPhotoEntity entity) {
        if (entity.isActivated()) {
            int ticksSinceActivation = entity.tickCount - entity.getActivationTick();
            if (ticksSinceActivation >= SWAP_TIME_IN_TICKS) {
                return MATCH_AFTER_TEXTURE;
            }
        }
    ...
    @Override
    public void render(NinthPhotoEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
