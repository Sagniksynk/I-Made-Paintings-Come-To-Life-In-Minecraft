...
public class SeventhPhotoRenderer extends EntityRenderer<SeventhPhotoEntity> {
    public static int SWAP_TIME_IN_TICKS = 12;

    private static final ResourceLocation WITHER_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/wither.png");
    private static final ResourceLocation WITHER_AFTER_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/witherr_after.png");

    public SeventhPhotoRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(SeventhPhotoEntity entity) {
        if (entity.isActivated()) {
            int ticksSinceActivation = entity.tickCount - entity.getActivationTick();
            if (ticksSinceActivation >= SWAP_TIME_IN_TICKS) {
                return WITHER_AFTER_TEXTURE;
            }
        }
        return WITHER_TEXTURE;
    }

    @Override
    public void render(SeventhPhotoEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
