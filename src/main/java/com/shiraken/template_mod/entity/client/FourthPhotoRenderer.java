...
public class FourthPhotoRenderer extends EntityRenderer<FourthPhotoEntity> {
    public static int SWAP_TIME_IN_TICKS = 15;

    private static final ResourceLocation PHOTO_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/fourth_photo.png");
    private static final ResourceLocation PHOTO_AFTER_TEXTURE = new ResourceLocation(TemplateMod.MODID, "textures/entity/fourth_photo_activated.png");

    public FourthPhotoRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(FourthPhotoEntity entity) {
        if (entity.isActivated()) {
            int ticksSinceActivation = entity.tickCount - entity.getActivationTick();
            if (ticksSinceActivation >= SWAP_TIME_IN_TICKS) {
                return PHOTO_AFTER_TEXTURE;
            }
        }
    ...
    @Override
    public void render(FourthPhotoEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
        ...
        final float FRONT_Z = -0.21875f; 
        final float BACK_Z  = -0.15625f;
