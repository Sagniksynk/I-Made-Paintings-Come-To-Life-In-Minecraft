...
public class FifthRoomEntity extends PathfinderMob implements GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final EntityDataAccessor<Boolean> IS_ACTIVATED = SynchedEntityData.defineId(FifthRoomEntity.class, EntityDataSerializers.BOOLEAN);

    public FifthRoomEntity(EntityType<? extends PathfinderMob> type, Level level) {
        super(type, level);
    }
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (!this.level().isClientSide && source.getEntity() instanceof Player) {
            this.spawnAtLocation(TemplateMod.FIFTH_ROOM_ITEM.get());
            this.discard();
            return true;
        }
        return false;
    ...
    private PlayState predicate(AnimationState<FifthRoomEntity> event) {
        if (this.isActivated()) {
            event.getController().setAnimation(RawAnimation.begin().thenLoop("Moving_hand"));
